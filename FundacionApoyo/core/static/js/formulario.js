...
const expresiones = {
	...
	edadMascota: /^\d{1,2}$/, // 1 a 2 numeros.
}
...
const validarFormulario = (e) => {
	switch (e.target.name) {
		...
		break;
		case "edadMascota":
			validarCampo(expresiones.edadMascota, e.target, 'edadMascota');
		break;
	}
}
...
formulario.addEventListener('submit', (e) => {
	...
	if(campos.usuario && campos.nombre && campos.apellidoPaterno && campos.apellidoMaterno && campos.password && campos.correo && campos.telefono && campos.mascota && campos.edadMascota && terminos.checked ){
		formulario.reset();
